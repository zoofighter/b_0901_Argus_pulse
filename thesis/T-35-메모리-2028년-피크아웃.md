...
```mermaid
graph LR
    P_T_02["[[T-02-메모리-산업의-변화|T-02 메모리 산업의 변화]]"] --> T_T_35["★ [[T-35-메모리-2028년-피크아웃|T-35 메모리 2028년 피크아웃]]"]
    P_T_06["[[T-06-TPU-증가와-GPU-수요-둔화|T-06 TPU 증가와 GPU 수요 둔화]]"] --> T_T_35["★ [[T-35-메모리-2028년-피크아웃|T-35 메모리 2028년 피크아웃]]"]
    P_T_07["[[T-07-CXL-메모리의-확대|T-07 CXL 메모리의 확대]]"] --> T_T_35["★ [[T-35-메모리-2028년-피크아웃|T-35 메모리 2028년 피크아웃]]"]
```

- 🔼 **선행 가설 (배경 요인 & 상위 인프라)**:
  - [[T-02-메모리-산업의-변화|T-02 메모리 산업의 변화]] — HBM4 양산 공급 확대
  - [[T-06-TPU-증가와-GPU-수요-둔화|T-06 TPU 증가와 GPU 수요 둔화]] — 빅테크 CAPEX 사이클
  - [[T-07-CXL-메모리의-확대|T-07 CXL 메모리의 확대]] — 메모리 대체제 출현
- ➡️ **동반 및 대체·경쟁 가설**:
  - (해당 없음)
- 🔽 **파생 및 후행 병목 가설**:
  - (해당 없음)
...
WHERE contains(thesis, "T-35") OR contains(file.outlinks, this.file.link)
