...
```mermaid
graph LR
    P_T_05["[[T4-01-금리와-데이터센터|T4-01 금리와 데이터센터]]"] --> T_T_37["★ [[T4-04-AI-버블-가능성|T4-04 AI 버블 가능성]]"]
    P_T_08["[[T4-02-엔비디아와-네오클라우드|T4-02 엔비디아와 네오클라우드]]"] --> T_T_37["★ [[T4-04-AI-버블-가능성|T4-04 AI 버블 가능성]]"]
    P_T_09["[[T4-03-엔비디아와-GPU-금융|T4-03 엔비디아와 GPU 금융]]"] --> T_T_37["★ [[T4-04-AI-버블-가능성|T4-04 AI 버블 가능성]]"]
```

- 🔼 **선행 가설 (배경 요인 & 상위 인프라)**:
  - [[T4-01-금리와-데이터센터|T4-01 금리와 데이터센터]] — CAPEX ROI 불일치
  - [[T4-02-엔비디아와-네오클라우드|T4-02 엔비디아와 네오클라우드]] — GPU 부채 리스크
  - [[T4-03-엔비디아와-GPU-금융|T4-03 엔비디아와 GPU 금융]] — 금융 레버리지 청산
- ➡️ **동반 및 대체·경쟁 가설**:
  - (해당 없음)
- 🔽 **파생 및 후행 병목 가설**:
  - (해당 없음)
...
WHERE contains(thesis, "T4-04") OR contains(thesis, "T4-04") OR contains(file.outlinks, this.file.link)
