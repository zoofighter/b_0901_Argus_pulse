...
```mermaid
graph LR
    P_T_02["[[T1-01-메모리-산업의-변화|T1-01 메모리 산업의 변화]]"] --> T_T_36["★ [[T6-02-중국반도체의-HBM-생산가능성|T6-02 중국 반도체의 HBM 생산 가능성]]"]
    T_T_36["★ [[T6-02-중국반도체의-HBM-생산가능성|T6-02 중국 반도체의 HBM 생산 가능성]]"] --> D_T_02["[[T1-01-메모리-산업의-변화|T1-01 메모리 산업의 변화]]"]
```

- 🔼 **선행 가설 (배경 요인 & 상위 인프라)**:
  - [[T1-01-메모리-산업의-변화|T1-01 메모리 산업의 변화]] — HBM3E 기술 성숙
- ➡️ **동반 및 대체·경쟁 가설**:
  - (해당 없음)
- 🔽 **파생 및 후행 병목 가설**:
  - [[T1-01-메모리-산업의-변화|T1-01 메모리 산업의 변화]] — 글로벌 HBM3E 마진 하방 압력
...
WHERE contains(thesis, "T6-02") OR contains(thesis, "T6-02") OR contains(file.outlinks, this.file.link)
