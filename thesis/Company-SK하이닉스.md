...
```mermaid
graph TD
    SK["🏢 SK하이닉스"] --> T02["[[T1-01-메모리-산업의-변화|T1-01 HBM4 커스텀 ASIC 전환]]"]
    SK --> T07["[[T1-03-CXL-메모리의-확대|T1-03 CXL 메모리 풀링]]"]
    SK --> T10["[[T1-04-첨단-패키징과-CoWoS의-병목|T1-04 첨단 패키징 & CoWoS]]"]
    SK --> T28["[[T1-10-3D-DRAM-기술-전환과-400단-V-NAND|T1-10 3D DRAM & V-NAND]]"]
    SK --> T33["[[T1-12-반도체-소부장의-내재화|T1-12 반도체 소부장 내재화]]"]
    SK --> T35["[[T1-13-메모리-2028년-피크아웃|T1-13 메모리 2028년 피크아웃]]"]
    SK -. 경쟁 .-> T36["[[T6-02-중국반도체의-HBM-생산가능성|T6-02 중국 HBM 생산 위협]]"]
```

1. **[[T1-01-메모리-산업의-변화|T1-01 메모리 산업의 변화 (핵심 수혜)]]**:
   - HBM4 커스텀 ASIC화에 따른 1~2년 LTA(장기공급계약) 체결로 분기별 단가 인하 사이클 탈피.
2. **[[T1-04-첨단-패키징과-CoWoS의-병목|T1-04 첨단 패키징과 CoWoS의 병목]]**:
   - TSMC의 CoWoS 캐파 우선 배정을 통해 패키징 병목 국면에서도 안정적 출하 유지.
3. **[[T1-03-CXL-메모리의-확대|T1-03 CXL 메모리의 확대]]**:
   - HBM 중심에서 CXL 2.0/3.0 기반 CMM-D(DRAM 풀링) 제품군 라인업 확장.
4. **[[T1-10-3D-DRAM-기술-전환과-400단-V-NAND|T1-10 3D DRAM 기술 전환]]**:
   - 1c/1d nm 미세화 한계 극복을 위한 3D D램 및 하이브리드 본딩 R&D 선도.
5. **[[T1-13-메모리-2028년-피크아웃|T1-13 메모리 2028년 피크아웃]]**:
