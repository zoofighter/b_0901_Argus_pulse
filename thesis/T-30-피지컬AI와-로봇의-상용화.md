...
```mermaid
graph LR
    P_T_03["[[T-03-전고체-배터리의-변화|T-03 전고체 배터리의 변화]]"] --> T_T_30["★ [[T-30-피지컬AI와-로봇의-상용화|T-30 피지컬 AI와 로봇의 상용화]]"]
    P_T_17["[[T-17-휴머노이드-로봇과-액추에이터-공급망|T-17 휴머노이드 로봇과 액추에이터 공급망]]"] --> T_T_30["★ [[T-30-피지컬AI와-로봇의-상용화|T-30 피지컬 AI와 로봇의 상용화]]"]
```

- 🔼 **선행 가설 (배경 요인 & 상위 인프라)**:
  - [[T-03-전고체-배터리의-변화|T-03 전고체 배터리의 변화]] — 차세대 로봇 배터리
  - [[T-17-휴머노이드-로봇과-액추에이터-공급망|T-17 휴머노이드 로봇과 액추에이터 공급망]] — 액추에이터·하드웨어 공급망
- ➡️ **동반 및 대체·경쟁 가설**:
  - (해당 없음)
- 🔽 **파생 및 후행 병목 가설**:
  - (해당 없음)
...
WHERE contains(thesis, "T-30") OR contains(file.outlinks, this.file.link)
