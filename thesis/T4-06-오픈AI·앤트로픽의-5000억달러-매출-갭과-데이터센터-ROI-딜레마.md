...
aliases:
- T4-06
- T4-06 오픈AI·앤트로픽의 5000억 달러 매출 갭과 데이터센터 ROI 딜레마
...
flowchart TD
    ...
    RevGap -->|해소 시나리오| Win_SaaS["[[T5-02-AI-소프트웨어-레이어의-과점화|T5-02 글로벌 SaaS 및 인건비 풀 완전 장악]]"]
    
    RevGap -->|미해소 리스크| Risk_CapEx["[[T4-04-AI-버블-가능성|T4-04 빅테크 CapEx 급격한 속도조절]]"]
    Risk_CapEx --> Risk_Chip["[[T1-02-TPU-증가와-GPU-수요-둔화|T1-02 GPU 고비용 탈피 및 자체 ASIC 가속]]"]
    Risk_CapEx --> Risk_Mem["[[T1-13-메모리-2028년-피크아웃|T1-13 HBM/메모리 슈퍼사이클 조기 피크아웃]]"]
    Risk_CapEx --> Risk_Rate["[[T4-01-금리와-데이터센터|T4-01 데이터센터 파이낸싱 부실화]]"]
```
