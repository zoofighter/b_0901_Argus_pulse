...
openai_file = THESIS_DIR / "Company-OpenAI.md"
openai_text = """---
...
```
"""
openai_file.write_text(openai_text.strip() + "\n", encoding="utf-8")
print("Updated Company-OpenAI.md with GPT-6 Astra")

# 2. Update Topic-프론티어모델.md
topic_f = THESIS_DIR / "Topic-프론티어모델.md"
topic_f_text = """---
...
## 📌 핵심 기술 개요
사전학습(Pre-training) 데이터 한계(Data Wall)에 직면한 AI 연구가 강화학습(RL) 기반의 **생각의 연쇄(Chain-of-Thought)** 및 **추론 시간 연산(Test-Time Compute)** 스케일링으로 패러다임을 전환하고 있습니다. 특히 2026년 9월 공개된 OpenAI의 **'GPT-6 아스트라(Astra)'**와 o1/o3, Anthropic Claude 3.5 Sonnet은 복잡한 소프트웨어 엔지니어링, 수학, 사이버보안 영역에서 인간 최고 수준의 전문가 역량을 완결적으로 대체하고 있습니다.

---

## 🔗 연관 테제 (Theses Mapping)
- **[[T4-06-GPT-6-아스트라와-자율형-AGI-엔지니어링-에이전트|T4-06 GPT-6 아스트라와 자율형 AGI 엔지니어링 에이전트]]**
...
"""
topic_f.write_text(topic_f_text.strip() + "\n", encoding="utf-8")
print("Updated Topic-프론티어모델.md")

# 3. Format Master MOC (46 theses)
master_moc = """---
...
> **Argus Nexus 중앙 사령탑**  
> AI 및 첨단 기술 산업의 구조적 변화를 추적하는 **총 46개 투자 가설(Theses)**의 상호 인과관계 맵과 **6대 섹터 & 5대 교차 차원(다차원 대시보드)** 사령탑입니다.  
...
> 46개 정식 테제 밖에서 새롭게 감지되어 검증·보육 중인 **차세대 후보 가설(`TC-XX`)** 목록입니다.
...
(THESIS_DIR / "00-Argus-Master-MOC.md").write_text(master_moc.strip() + "\n", encoding="utf-8")
if (DOCS_DIR / "00-Argus-Master-MOC.md").parent.exists():
    (DOCS_DIR / "00-Argus-Master-MOC.md").write_text(master_moc.strip() + "\n", encoding="utf-8")
print("Updated 00-Argus-Master-MOC.md with 46 theses")
